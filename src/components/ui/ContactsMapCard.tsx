...
export default function ContactsMapCard({ showHeading = true }: { showHeading?: boolean }) {
  return (
    <section className="bg-white">
      ...
      <div className="relative h-[680px] lg:h-[720px] overflow-hidden">
        {/* Карта-фон */}
        <img src={mapBg} alt="" className="absolute inset-0 w-full h-full object-cover" />

        {/* Фото клиники на карте */}
        <div className="hidden lg:block absolute left-[55%] top-[32%] w-64 h-44 rounded-2xl overflow-hidden shadow-2xl border-4 border-white">
          <img src={imgClinicBuilding} alt="Erensau Hospital" className="w-full h-full object-cover" />
        </div>

        {/* Карточка контактов поверх карты */}
        <div className="absolute inset-0 container-main py-10 lg:py-14 pointer-events-none">
          <div className="bg-white rounded-3xl p-7 lg:p-9 shadow-2xl pointer-events-auto max-w-lg w-full">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-6 mb-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  <Dot><PhoneIcon /></Dot>
                  <a href="tel:+77273394040" className="text-sm text-gray-600 hover:text-[#00b5e2] transition-colors">+7 727 339 40 40</a>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex gap-1"><Dot><WaIcon /></Dot><Dot><TgIcon /></Dot></div>
                  <a href="tel:+77713999444" className="text-sm text-gray-600 hover:text-[#00b5e2] transition-colors">+7 771 399 94 44</a>
                </div>
              </div>
              {/* Доп.информация */}
              <div>
                <h3 className="text-gray-900 font-bold text-sm mb-2">Доп.информация:</h3>
                <div className="flex items-center gap-2 mb-1.5">
                  <div className="flex gap-1"><Dot><GlobeIcon /></Dot><Dot><IgIcon /></Dot></div>
                  <span className="text-sm text-gray-600">erensau.kz</span>
                </div>
                <div className="flex items-center gap-2 mb-1.5">
                  <Dot><IgIcon /></Dot>
                  <span className="text-sm text-gray-600">erensau</span>
                </div>
                <div className="flex items-center gap-2">
                  <Dot><MailIcon /></Dot>
                  <a href="mailto:info@erensau.kz" className="text-sm text-gray-600 hover:text-[#00b5e2] transition-colors">info@erensau.kz</a>
                </div>
              </div>
              {/* Администрация */}
              <div>
                <h3 className="text-gray-900 font-bold text-sm mb-2">Администрация:</h3>
                <div className="flex items-center gap-2 mb-1.5">
                  <Dot><PhoneIcon /></Dot>
                  <a href="tel:+77273394282" className="text-sm text-gray-600 hover:text-[#00b5e2] transition-colors">+7 727 339 42 82</a>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex gap-1"><Dot><WaIcon /></Dot><Dot><TgIcon /></Dot></div>
                  <a href="tel:+77717044848" className="text-sm text-gray-600 hover:text-[#00b5e2] transition-colors">+7 771 704 48 48</a>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-gray-100 pt-5 mb-6">
              <h3 className="text-[#00b5e2] font-bold text-sm mb-3">График работы</h3>
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-gray-400 text-xs">
                    <th className="text-left font-medium pb-2">День недели</th>
                    <th className="text-left font-medium pb-2">Клиника</th>
                ...
                <tbody>
                  {schedule.map((row) => (
                    <tr key={row.day} className="text-gray-700">
                      ...
                      <td className="py-1">{row.clinic}</td>
                      <td className="py-1">{row.call}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
